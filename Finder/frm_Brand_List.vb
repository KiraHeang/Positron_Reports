...
Public Class frm_Brand_List
    ...
    Private Sub btn_Exit_Click(sender As Object, e As EventArgs) Handles btn_Exit.Click
        Me.Close()
    End Sub
    Private Sub frm_Brand_List_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            End If
            Dim V_TBL As DataTable = SQL_GetTable("Select ROW_NUMBER() OVER (ORDER BY Code) AS No, Code,Name from [@BIZ_TB_BRAND]")
        ...
        Try
            ...
            For Each row As DataGridViewRow In Data_GridView.Rows
                If Convert.ToBoolean(row.Cells("gc_check").Value) = True Then
                    selectedCodes.Add(row.Cells("gc_brand_code").Value.ToString().Trim())
                End If
            Next
            ...
            If selectedCodes.Count = 0 Then
                ...
                Exit Sub
            End If
            F_Brand_Code = String.Join(",", selectedCodes)
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub Data_GridView_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles Data_GridView.CellDoubleClick
        Try
            If Data_GridView.RowCount = 0 Then
                Exit Sub
            End If
            F_Brand_Code = Data_GridView.CurrentRow.Cells("gc_brand_code").Value.ToString.Trim
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
        ...
        Try
            ...
            If cmb_FindBy.SelectedIndex = 0 Then
                If txt_Filter.Text = "" Then
                    v_SQL = "Select ROW_NUMBER() OVER (ORDER BY Code) AS No, Code,Name from [@BIZ_TB_BRAND]"
                Else
                    v_SQL = "select ROW_NUMBER() OVER (ORDER BY Code) AS No,Code,Name from [@BIZ_TB_BRAND] Where Code LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY Code"
                End If
            ElseIf cmb_FindBy.SelectedIndex = 1 Then
                If txt_Filter.Text = "" Then
                    v_SQL = "Select ROW_NUMBER() OVER (ORDER BY Code) AS No, Code,Name from [@BIZ_TB_BRAND]"
                Else
                    v_SQL = "select ROW_NUMBER() OVER (ORDER BY Code) AS No,Code,Name from [@BIZ_TB_BRAND] WHERE Name LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY Code"
                End If
            End If
