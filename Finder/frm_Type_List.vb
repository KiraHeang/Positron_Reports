...
Public Class frm_Type_List
    ...
    Private Sub frm_Type_List_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            End If
            Dim V_TBL As DataTable = SQL_GetTable("SELECT ROW_NUMBER() OVER (ORDER BY Code) AS No, Code, Name FROM [@BIZ_TB_TYPE]")
        ...
        Try
            ...
            For Each row As DataGridViewRow In Data_GridView.Rows
                If Convert.ToBoolean(row.Cells("gc_check").Value) = True Then
                    selectedCodes.Add(row.Cells("gc_code").Value.ToString().Trim())
                End If
            Next

            If selectedCodes.Count = 0 Then
                MessageBox.Show("Please check at least one type.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If
            F_TypeCode = String.Join(",", selectedCodes)
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub Data_GridView_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles Data_GridView.CellDoubleClick
        Try
            If Data_GridView.RowCount = 0 Then Exit Sub
            F_TypeCode = Data_GridView.CurrentRow.Cells("gc_code").Value.ToString.Trim
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub btn_find_Click(sender As Object, e As EventArgs) Handles btn_find.Click
        Try
            ...
            End If
            Dim v_SQL As String = ""
            If cmb_FindBy.SelectedIndex = 0 Then ' Code
                If txt_Filter.Text = "" Then
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY Code) AS No, Code, Name FROM [@BIZ_TB_TYPE]"
                Else
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY Code) AS No, Code, Name FROM [@BIZ_TB_TYPE] WHERE Code LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY Code"
                End If
            ElseIf cmb_FindBy.SelectedIndex = 1 Then ' Name
                If txt_Filter.Text = "" Then
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY Code) AS No, Code, Name FROM [@BIZ_TB_TYPE]"
                Else
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY Code) AS No, Code, Name FROM [@BIZ_TB_TYPE] WHERE Name LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY Code"
                End If
            End If
