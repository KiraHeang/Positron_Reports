...
Public Class frm_Items
        ...
        Try
            ...
            If cmb_FindBy.SelectedIndex = 0 Then
                If txt_Filter.Text = "" Then
                    v_SQL = "Select ROW_NUMBER() OVER (ORDER BY ItemCode) AS No, ItemCode,ItemName from OITM Order By ItemCode"
                Else
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY ItemCode) AS No, ItemCode, ItemName FROM OITM WHERE ItemCode LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY ItemCode"
                End If
            ElseIf cmb_FindBy.SelectedIndex = 1 Then
                If txt_Filter.Text = "" Then
                    v_SQL = "Select ROW_NUMBER() OVER (ORDER BY ItemCode) AS No, ItemCode,ItemName from OITM Order By ItemCode"
                Else
                    v_SQL = "SELECT ROW_NUMBER() OVER (ORDER BY ItemCode) AS No, ItemCode, ItemName FROM OITM WHERE ItemName LIKE '%" & txt_Filter.Text.Trim & "%' ORDER BY ItemCode"
                End If
            End If
    ...
    Private Sub Data_GridView_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles Data_GridView.CellDoubleClick
        Try
            If Data_GridView.RowCount = 0 Then
                Exit Sub
            End If
            F_Item = Data_GridView.CurrentRow.Cells("gc_item_code").Value.ToString.Trim
            Me.Close()
        Catch ex As Exception
            MessageBox.Show(ex.Message, V_ProjectName, MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub frm_Items_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            Dim V_TBL As DataTable = SQL_GetTable("Select ROW_NUMBER() OVER (ORDER BY ItemCode) AS No, ItemCode,ItemName from OITM Order By ItemCode")
            If V_TBL IsNot Nothing Then
                For Each DR As DataRow In V_TBL.Rows
                    Data_GridView.Rows.Add(DR("No").ToString.Trim,
                                           DR("ItemCode").ToString.Trim,
